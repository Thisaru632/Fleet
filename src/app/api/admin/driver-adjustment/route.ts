...
export async function POST(request: Request) {
  try {
    ...
    const { driverCode, month, salaryAdvance, shorts, excess, inclusions, exclusions, comment, exclusionsComment } = body;
    ...
    const numExcess = Number(excess) || 0;
    ...
    const updated = await DriverAdjustment.findOneAndUpdate(
      ...
      { $set: { salaryAdvance: numAdvance, shorts: numShorts, excess: numExcess, inclusions: numInclusions, exclusions: numExclusions, comment: commentStr, exclusionsComment: exclusionsCommentStr } },
